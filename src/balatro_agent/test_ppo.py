...
class PPOEvaluator:
    ...
    def evaluate(self, num_episodes: int = 100) -> Dict[str, Any]:
        ...
        action_distribution = {
            "play": (
                f"{action_counts['play']/total_actions:.1%}"
                if total_actions > 0
                else "0%"
            ),
            "discard": (
                f"{action_counts['discard']/total_actions:.1%}"
                if total_actions > 0
                else "0%"
            ),
        }
    ...
    def _print_hand_formatted(
        self, hand_to_print, prefix: str = "Current hand:", suffix: str = ""
    ):
        """
        Formats and prints a list of card objects (a hand) with suit icons and colors.

        Args:
            hand_to_print: A list of card objects, where each card has 'suit' and 'rank' attributes.
            prefix: A string to prepend to the hand display (e.g., "Initial hand:").
        """
        ...
        for card in hand_to_print:
            rank = card.rank if hasattr(card, "rank") else card[0:-1]
        ...
        print(f"{BOLD}{prefix}{RESET_COLOR} [{', '.join(formatted_cards)}] {suffix}")
    ...
    def demo_episodes(self, num_episodes: int = 3, max_steps: int = 15):
        ...
        for episode in range(num_episodes):
            ...
            self._print_hand_formatted(self.env.hand, prefix="Initial hand:")
            ...
            while step < max_steps:
                ...
                print(f"Step {step} - {self._print_hand_formatted(self.env.hand, prefix='Current hand:')}")
                ...
                cards_to_play = [
                    ...
                    if i < len(self.env.hand)
                ]
                
                # Capture top 3 action cards BEFORE taking the main action
                top_action_cards = []
                for i in range(3):
                    top_action_type, top_card_indices = self.env._decode_action(
                        top_action_indices[i]
                    )
                    top_cards_str = [
                        str(self.env.hand[j])
                        for j in top_card_indices
                        if j < len(self.env.hand)
                    ]
                    top_action_cards.append((top_action_type, top_cards_str))
                ...
                cards_str = cards_to_play
                self._print_hand_formatted(
                    cards_str, prefix=f"  Action: {action_type.upper()} "
                )
                print(f"  Probability: {action_prob:.3f}")
                print(f"  Reward: {reward:.2f}")
                print(f"  Score: {self.env.current_score}/{self.env.blind_score}")
                print(
                    f"  Plays left: {self.env.plays_left}, Discards left: {self.env.discards_left}"
                )
                ...
                for i in range(3):
                    top_action_type, top_cards_str = top_action_cards[i]
                    self._print_hand_formatted(
                        top_cards_str,
                        prefix=f"    {i+1}. {top_action_type.upper()} ",
                        suffix=f" (prob: {top_action_probs[i]:.3f})",
                    )
    ...
    def analyze_policy(self, num_samples: int = 1000):
        ...
        for _ in range(num_samples):
            ...
            with torch.no_grad():
                ...
                action_preferences[action_type] = (
                    action_preferences.get(action_type, 0) + 1
                )
        ...
        action_percentages = {
            k: v / total_samples * 100 for k, v in action_preferences.items()
        }
    ...
    def plot_evaluation_results(
        self, eval_stats: Dict[str, Any], save_path: str = "evaluation_results.png"
    ):
        ...
        axes[0, 2].set_title(
            "Episode Length Distribution", fontsize=14, fontweight="bold"
        )
        ...
        if eval_stats["hand_type_distribution"]:
            ...
            axes[1, 1].set_title(
                "Hand Type Distribution", fontsize=14, fontweight="bold"
            )
        ...
        axes[1, 2].text(
            0.1,
            0.8,
            f'Win Rate: {eval_stats["win_rate"]*100:.2f}%',
            fontsize=12,
            transform=axes[1, 2].transAxes,
        )
        axes[1, 2].text(
        ...
        axes[1, 2].text(
            0.1,
            0.5,
            f'Avg Final Score: {eval_stats["avg_final_score"]:.1f}',
            fontsize=12,
            transform=axes[1, 2].transAxes,
        )
        axes[1, 2].text(
        ...
        axes[1, 2].text(
            0.1,
            0.3,
            f'Discard Ratio: {action_dist["discard"]}',
            fontsize=12,
            transform=axes[1, 2].transAxes,
        )
        axes[1, 2].axis("off")
...
def main():
    ...
    parser.add_argument(
        "--blind-score", type=int, default=300, help="Target score for evaluation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--mode",
        choices=["evaluate", "demo", "analyze", "all"],
        default="all",
    ...
    parser.add_argument(
        "--num-demos", type=int, default=3, help="Number of demo episodes"
    )
